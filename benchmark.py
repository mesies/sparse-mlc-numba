...
for n in N:
    t = tic()
    mlc = MlcLinReg(learning_rate=0.02, iterations=100, batch_size=n, verbose=False, sparse=True)
    ...
    t = tic()
    clf = SGDClassifier(loss='log', max_iter=1000, tol=1e-3, verbose=0, shuffle=True)
...
plt.plot(N, times, label="Minibatch")
